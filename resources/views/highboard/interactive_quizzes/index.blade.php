        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('highboard.interactive_quizzes.create') }}" class="btn btn-primary">
                            ...
                            <tbody>
                                @forelse($quizzes as $quiz)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('highboard.interactive_quizzes.leaderboard', $quiz) }}" class="btn btn-info btn-sm" title="View Leaderboard">
                                                <i class="fas fa-trophy"></i> Leaderboard
                                            </a>
                                            <a href="#" class="btn btn-outline-danger btn-sm copyQuizLink" data-clipboard-text="{{ route('interactive_quiz.show', $quiz) }}">
                                                <i class="fe fe-copy"></i> Copy Link
                                            </a> 
                                            <a href="#" class="btn btn-outline-dark btn-sm download-qr" data-url="{{ route('interactive_quiz.show', $quiz) }}" data-name="quiz-{{ $quiz->id }}-qr">
                                                <i class="fas fa-qrcode"></i> QR
                                            </a> 
                                            <a href="{{ route('highboard.interactive_quizzes.show', $quiz) }}" class="btn btn-outline-info btn-sm">View</a>
                                            <a href="{{ route('highboard.interactive_quizzes.edit', $quiz) }}" class="btn btn-outline-primary btn-sm">Edit</a>
                                            <form action="{{ route('highboard.interactive_quizzes.toggle-active', $quiz) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('PATCH')
                                            ...
                                            <form action="{{ route('highboard.interactive_quizzes.destroy', $quiz) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('DELETE')
