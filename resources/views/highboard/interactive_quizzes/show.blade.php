        ...
        <div class="col-lg-12">
            <div class="d-flex align-items-center justify-content-between mb-3">
                ...
                <div class="d-flex gap-2">
                    <a  class="btn btn-primary" id="copyQuizLink" data-clipboard-text="{{ route('interactive_quiz.show', $quiz) }}">
                        <i class="fas fa-copy me-1"></i> Copy Link
                    </a>
                    <a href="{{ route('highboard.interactive_quizzes.leaderboard', $quiz) }}" class="btn btn-info">
                        <i class="fas fa-trophy me-1"></i> Leaderboard
                    </a>
                    <form action="{{ route('highboard.interactive_quizzes.toggle-active', $quiz) }}" method="POST">
                        @csrf
                        @method('PATCH')
                    ...
                    <a href="{{ route('highboard.interactive_quizzes.index') }}" class="btn btn-outline-secondary">Back</a>
                </div>
            </div>
            ...
            <div class="card shadow-sm">
                <div class="card-header bg-white d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <a href="{{ route('highboard.quizzes.questions.create', $quiz) }}" class="btn btn-primary btn-sm">Add Question</a>
                    </div>
                </div>
                            ...
                            <tbody>
                                @forelse ($quiz->questions as $question)
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            <a href="{{ route('highboard.questions.edit', $question) }}" class="btn btn-outline-primary btn-sm">Edit</a>
                                            <form action="{{ route('highboard.questions.destroy', $question) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('DELETE')
    ...
    <script>
        ...
        function importQuestionsAi() {
            Swal.fire({
                ...
                preConfirm: (text) => {
                    return fetch('{{ route('highboard.quizzes.questions.ai-import', $quiz) }}', {
                        method: 'POST',
                        headers: {
