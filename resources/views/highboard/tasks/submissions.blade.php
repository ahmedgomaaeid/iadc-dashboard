...
@section('content')
    ...
    <div class="page-header">
        ...
        <div>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('highboard.dashboard') }}"><i class="fe fe-home"></i> Dashboard</a></li>
                <li class="breadcrumb-item"><a href="{{ route('highboard.tasks.index') }}"><i class="fe fe-check-square"></i> Tasks</a></li>
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                ...
                <div class="card-body">
                    <!-- Filter Form -->
                    <form method="GET" action="{{ route('highboard.tasks.submissions') }}" class="mb-4">
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label class="form-label">Filter by Committee</label>
                                    <select name="committee_id" class="form-control" onchange="this.form.submit()">
                                        <option value="">All Committees</option>
                                        @foreach($committees as $committee)
                                            <option value="{{ $committee->id }}" {{ $committeeId == $committee->id ? 'selected' : '' }}>
                                                {{ $committee->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <tbody>
                                    @foreach($submissions as $submission)
                                        <tr>
                                            ...
                                            <td>
                                                <span class="badge bg-info">{{ $submission->task->committee->name }}</span>
                                            </td>
                                            <td>
                                                <a href="{{ route('highboard.tasks.show', $submission->task) }}">
